...
# checkov:skip=CKV_AWS_91: Customer will define if access logging is enabled/disabled when consuming the module  
...
# checkov:skip=CKV_AWS_2: Customer will define the ALB protocol when consuming the module  
...
resource "aws_lb_target_group_attachment" "this" {
  for_each = {
    ...
    k => v if local.create && lookup(v, "create_attachment", true) &&
           (try(aws_lb_target_group.this[k].target_type, null) != "ip")
  }

  target_group_arn  = aws_lb_target_group.this[each.key].arn
  target_id         = try(each.value.target_id, null)
  port              = try(aws_lb_target_group.this[each.key].target_type, null) == "lambda" ? null : try(each.value.port, var.default_port)
